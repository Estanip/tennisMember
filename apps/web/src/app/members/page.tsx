...
import type { Member, MemberCondition, MemberStatus, PaginatedMembers } from "@socios/shared";
import {
...
export default function MembersPage() {
  ...
  async function handleDelete(member: Member) {
    if (!isAdmin) return;
    const confirmed = window.confirm(`¿Eliminar a ${member.fullName}? (soft delete)`);
    if (!confirmed) return;
    try {
                    ...
                    data.items.map((member) => (
                      <tr key={member.id}>
                        ...
                        <td>
                          <span
                            className={`badge ${
                              member.status === MEMBER_STATUS.ENABLED
                                ? "enabled"
                        ...
                        {isAdmin ? (
                          <td>
                            <div className="actions">
                              <Link className="btn btn-secondary" href={`/members/${member.id}`}>
                                Editar
                              </Link>
                              <button
                                type="button"
